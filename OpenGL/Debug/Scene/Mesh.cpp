...
void Mesh::shadingPass(const ShadowMap* sm, mat4& vp, vec3 c, float f) {
	shader->useShader();
	setUniformsForShadingPass(vp, c, sm->light_id, f);
	draw();
	glUseProgram(0);

}
void Mesh::setUniformsForShadingPass(mat4& vp, vec3 c, int light_id, float far) {
	...
	// Far plane
	auto far_location = glGetUniformLocation(shader->programHandle, "far_plane");
...
void Mesh::rotateLocally(float deltaTime) {
	// Use Blender coordinate system for local transformations
	model_matrix *= rotate(mat4(1.0), deltaTime*1.0f, glm::vec3(0.0f, 0.0f, 1.0f)); // model matrix moved to geometry
}
...
//void Mesh::setCameraLoc(glm::vec3 c){
//	shader->useShader();
//	
//	glUseProgram(0);
//}
